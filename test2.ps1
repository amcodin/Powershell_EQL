## Fixing backup button not working, restore works, added checkboxes to listview
    ...
    @(
        ...
        if (Test-Path -Path $_ -PathType Container) {
            $result.Add([PSCustomObject]@{
                ...
                Path = $_
                Type = "Folder"
                # IsSelected = $true # Add IsSelected for consistency, useful if checkboxes are ever added to Backup mode
            })
        } else {
            Write-Verbose "Standard folder not found or inaccessible: $_"
        }
    }
