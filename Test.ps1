# Working but no checkboxes nextr to restore options
...
function Get-BackupPaths {
    ...
    try {
        if (Test-Path $chromePath -PathType Leaf) {
            # Test read access silently
            Get-Content $chromePath -TotalCount 1 -ErrorAction Stop | Out-Null
            $result.Add([PSCustomObject]@{
                Name = "Chrome Bookmarks" # Use a descriptive name
                Path = $chromePath
                Type = "File"
            })
        }
    ...
    $edgePath = "$env:LOCALAPPDATA\Microsoft\Edge\User Data\Default\Bookmarks"
     try {
        if (Test-Path $edgePath -PathType Leaf) {
            # Test read access silently
            Get-Content $edgePath -TotalCount 1 -ErrorAction Stop | Out-Null
            $result.Add([PSCustomObject]@{
                Name = "Edge Bookmarks" # Use a descriptive name
                Path = $edgePath
                Type = "File"
            })
        }
...
function Show-MainWindow {
    ...
    # XAML UI Definition (Moved inside function to ensure it's always available)
    [xml]$XAML = @'
<Window
    ...
    <Grid>
        ...
        <ListView Name="lvwFiles" Margin="10,110,200,140" SelectionMode="Extended"> <!-- Changed to Extended for multi-select remove -->
             <ListView.View>
                <GridView>
...
'@
    ...
    try {
        ...
        # Disable Add/Remove buttons in Restore mode initially
        if (-not $IsBackup) {
            $controls.btnAddFile.IsEnabled = $false
            $controls.btnAddFolder.IsEnabled = $false
            $controls.btnRemove.IsEnabled = $false
        }
        ...
        if ($IsBackup) {
            ...
            $controls.lvwFiles.ItemsSource = $paths # Bind directly if Get-BackupPaths returns a collection
        }
        elseif (Test-Path $defaultPath) {
            ...
            if ($latestBackup) {
                ...
                $controls.lvwFiles.ItemsSource = $backupItems
            } else {
        ...
        $controls.btnAddFile.Add_Click({
            if (-not $IsBackup) { return } # Should be disabled, but double-check
            ...
            if ($result -eq [System.Windows.Forms.DialogResult]::OK) {
                # Add to the existing ItemsSource collection
                $currentItems = [System.Collections.Generic.List[PSCustomObject]]::new($controls.lvwFiles.ItemsSource)
                ...
                $controls.lvwFiles.ItemsSource = $currentItems
            }
        })
        ...
        $controls.btnAddFolder.Add_Click({
            ...
            $dialog.ShowNewFolderButton = $false # Don't allow creating folders here
        ...
        $controls.btnRemove.Add_Click({
            ...
            if ($controls.lvwFiles.SelectedItems.Count -gt 0) {
                $currentItems = [System.Collections.Generic.List[PSCustomObject]]::new($controls.lvwFiles.ItemsSource)
                $itemsToRemove = [System.Collections.Generic.List[PSCustomObject]]::new($controls.lvwFiles.SelectedItems)
        ...
        $controls.btnStart.Add_Click({
            ...
            if ([string]::IsNullOrEmpty($location) -or -not (Test-Path $location)) {
                [System.Windows.MessageBox]::Show("Please select a valid location first.", "Location Required", [System.Windows.MessageBoxButton]::OK, [System.Windows.MessageBoxImage]::Warning)
                return
            }
            ...
            try {
                ...
                if ($IsBackup) {
                    ...
                    $itemsToBackup = $controls.lvwFiles.ItemsSource
                    ...
                    $controls.prgProgress.Maximum = $totalFilesEstimate
                ...
                # ===================
                    ...
                    $totalFilesEstimate = $backupLog.Count
                    if ($controls.chkNetwork.IsChecked) { $totalFilesEstimate++ }
                    if ($controls.chkPrinters.IsChecked) { $totalFilesEstimate++ }
