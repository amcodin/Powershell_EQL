...
# Global variable to hold transfer log data, primarily for Express mode but could be adapted
$script:transferLog = [System.Collections.Generic.List[string]]::new()
...
# Moved to global scope so it can be called from anywhere, including main script block
Function LogTransfer ($Action, $Status, $Details) {
    ...
    # Add to the global log variable
        ...
        elseif (Test-Path $script:DefaultPath) { # Restore Mode
            ...
            if ($latestBackup) {
                ...
                $backupItems = Get-ChildItem -Path $latestBackup.FullName |
                    ...
                    Where-Object { $_.Name -notmatch '^(FileList_.*\.csv|TransferLog\.csv|ExpressTransferLog\.csv)$' } | # Exclude logs (Added Express log)
                    ForEach-Object {
                        [PSCustomObject]@{
                ...
                try {
                    Import-Csv $drivesCsvPath | ForEach-Object {
                        ...
                        if ($driveLetter -match '^[A-Z]$' -and $networkPath -match '^\\\\') {
                            $drivePathLocal = "$($driveLetter):" # Use different variable name to avoid confusion
                            if (-not (Test-Path -LiteralPath $drivePathLocal)) {
                                try {
...
function Execute-ExpressModeLogic {
    ...
    # Log data is now stored in $script:transferLog (global)
    ...
    # --- Initialize Log ---
    # Clear previous log entries if any and add header
    $script:transferLog.Clear()
    $script:transferLog.Add("Timestamp,Action,Status,Details")
    LogTransfer "Initialization" "Info" "Starting Express Mode Execution."
    ...
    try {
        ...
        try {
            ...
            # Use the global log variable
            $script:transferLog | ConvertTo-Csv -NoTypeInformation -Delimiter ',' | Set-Content -Path $logFilePath -Encoding UTF8 -Force
            ...
            try { $script:transferLog | ConvertTo-Csv -NoTypeInformation -Delimiter ',' | Set-Content -Path $altTempLog -Encoding UTF8 -Force; Write-Warning "Saved log to alternate temp location: $altTempLog" } catch {}
...
try {
    ...
    switch ($operationMode) {
        ...
        'Express' {
            ...
            if (-not $isAdmin) {
                ...
                # Use the globally defined LogTransfer function
                LogTransfer "Elevation" "Attempt" "Relaunching script as Administrator"
                try {
                    ...
                    Write-Error $errMsg
                    # Use the globally defined LogTransfer function
                    LogTransfer "Elevation" "Fatal Error" $errMsg
                ...
                # Use the globally defined LogTransfer function - THIS IS THE CALL THAT CAUSED THE ERROR
                LogTransfer "Elevation" "Success" "Script is running as Administrator"
                ...
                try {
                    ...
                    # Prompt requires interaction, cannot easily get $targetDevice here yet.
                    # We will get $targetDevice inside Execute-ExpressModeLogic first.
                    $credential = Get-Credential -UserName $currentUser -Message "Enter Administrator credentials for the OLD (source) computer" -ErrorAction Stop
                } catch {
    ...
    if ($_.FullyQualifiedErrorId -notmatch 'Express Transfer Failed') {
        ...
        # Log the fatal error using the global logger if possible
        try { LogTransfer "Fatal Error" "Script Level" $errorMessage } catch {}
        try {
