# Working, but the logging for the triggers isn't showing
    ...
    foreach ($path in $specificPaths) {
        if (Test-Path -Path $path) {
            $result.Add([PSCustomObject]@{
             ...
             Write-Host "specificPaths not found: $path"
        }
    }
...
# --- REMOVED Global Definitions for Set-GPupdate and Start-ConfigManagerActions ---
# --- They are now defined ONLY inside the Start-Job block in Show-ModeDialog ---
...
function Show-ModeDialog {
    ...
    if (-not $isBackupMode) {
        Write-Host "Restore mode selected. Initiating background system updates job..."
        ...
        $updateJob = Start-Job -Name "BackgroundUpdates" -ScriptBlock {
            ...
            function Start-ConfigManagerActions {
                 ...
                 if (-not (Test-Path $ccmExecPath)) {
                     Write-Warning "JOB: Configuration Manager client not found at $ccmExecPath. Skipping CM actions."
                     Write-Host "JOB: Exiting Start-ConfigManagerActions function (CM client not found)."
                     return $true # Return success as CM is not applicable
                 }
                 Write-Host "JOB: CM Client found at $ccmExecPath."
                 ...
                 try {
                     ...
                     if (Get-CimClass -Namespace $clientSDKNamespace -ClassName $clientClassName -ErrorAction SilentlyContinue) {
                          Write-Host "JOB: Using CIM method to trigger schedules."
                          $attemptedMethod = "WMI/CIM"
                          ...
                          Write-Warning "JOB: CIM Class $clientClassName not found in $clientSDKNamespace. Falling back to ccmexec.exe."
                          ...
                          foreach ($action in $scheduleActions) {
                             ...
                             try {
                                 ...
                                 if ($process.ExitCode -ne 0) {
                                     Write-Warning "JOB:   $($action.Name) action via ccmexec.exe finished with exit code $($process.ExitCode)"
                     ...
                     Write-Error "JOB: An unexpected error occurred during Configuration Manager actions ($attemptedMethod): $($_.Exception.Message)"
                     Write-Host "JOB: Exiting Start-ConfigManagerActions function (Error)."
                     return $false # Return failure on major error
                 }

                 Write-Host "JOB: Configuration Manager actions attempt finished. Overall success: $success."
            ...
            Write-Host "JOB: Initiating Group Policy update..." -ForegroundColor Cyan
            Set-GPupdate
            Write-Host "JOB: Entering Start-ConfigManagerActions function." -ForegroundColor Cyan
            Start-ConfigManagerActions
        }
        Write-Host "Background update job started (ID: $($updateJob.Id)). Main window will load."  -ForegroundColor Cyan
...
'@
    ...
    try {
        ...
        $controls.btnAddFile.Add_Click({
            ...
            # Button is always enabled now, but logic should still only apply in backup mode conceptually
            # if (-not $IsBackup) { Write-Host "Ignoring Add File click (not in Backup mode)."; return } # Keep check? Or allow adding anytime? Let's allow adding anytime for now.
        ...
        $controls.btnAddFolder.Add_Click({
             ...
             # if (-not $IsBackup) { Write-Host "Ignoring Add Folder click (not in Backup mode)."; return }
            $dialog = New-Object System.Windows.Forms.FolderBrowserDialog
        ...
        $controls.btnRemove.Add_Click({
            ...
            # Button is always enabled now.
            # if (-not $IsBackup) { Write-Host "Ignoring Remove click (not in Backup mode)."; return }
        ...
        $controls.btnStart.Add_Click({
            ...
            try {
                ...
                if ($IsBackup) {
                    ...
                    # --- MODIFIED: Filter ItemsSource based on IsSelected ---
                ...
                # ===================
                    ...
                    Write-Host "Background updates (GPUpdate/CM) should have been initiated earlier."
                ...
                # Re-enable UI elements
                 ...
                 # --- REMOVED: Restore Add/Remove button state based on mode ---
                 # $controls.btnAddFile.IsEnabled = $IsBackup
                 # $controls.btnAddFolder.IsEnabled = $IsBackup
                 # $controls.btnRemove.IsEnabled = $IsBackup
                 # Write-Host "Add/Remove button states reset."
