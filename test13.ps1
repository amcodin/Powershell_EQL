...
#
#
# Next version.  Requires admin access on execution.  Will prompt for OLD Device Asset number.  Creates map network drive to old device C$.  Moves required files from old device to new device
###########################################################################################################################
...
function Start-BackgroundUpdateJob {
    ...
    $script:updateJob = Start-Job -Name "BackgroundUpdates" -ScriptBlock {
        ...
        function Set-GPupdate {
            ...
            try {
                # FIX: Specify WorkingDirectory to avoid UNC path issues
        ...
        function Start-ConfigManagerActions {
             ...
             # FIX: Correct WMI/CIM Namespace and Class for TriggerSchedule
             ...
             $scheduleActions = @(
                 ...
                 @{ID = '{00000000-0000-0000-0000-000000000022}'; Name = 'User Policy Retrieval & Evaluation Cycle'}, # Note: This is actually Machine Policy Eval, User is 26/27
                 ...
                 @{ID = '{00000000-0000-0000-0000-000000000101}'; Name = 'Hardware Inventory Collection Cycle'}, # Often redundant with 0001
                 @{ID = '{00000000-0000-0000-0000-000000000108}'; Name = 'Software Updates Assignments Evaluation Cycle'},
                 @{ID = '{00000000-0000-0000-0000-000000000102}'; Name = 'Software Inventory Collection Cycle'} # Often redundant with 0002
             )
             ...
             # Method 1: WMI/CIM (Corrected)
             Write-Host "JOB: Attempting Method 1: Triggering via CIM ($wmiNamespace -> $wmiClassName)..."
             $cimMethodSuccess = $true # Assume success unless specific errors occur
             try {
                 # No need to check Get-CimClass first, just try Invoke-CimMethod
                 ...
                 foreach ($action in $scheduleActions) {
                    ...
                    try {
                        # Use the correct Namespace and ClassName
                        ...
                        # Log specific failure for this action
                 ...
                 # Check if all actions succeeded via CIM
                 if ($cimMethodSuccess) {
                 ...
                 # Catch unexpected errors during the overall CIM attempt (e.g., WMI service stopped)
                 Write-Error "JOB: An unexpected error occurred during CIM attempt: $($_.Exception.Message)"
                 $cimMethodSuccess = $false # Mark CIM method as failed
             }

             # Method 2: Fallback via ccmexec.exe (If CIM failed)
             if (-not $cimAttemptedAndSucceeded) {
                 ...
                 if (Test-Path -Path $ccmExecPath -PathType Leaf) {
                     ...
                     $execMethodSuccess = $true # Assume success unless execution fails
                     foreach ($action in $scheduleActions) {
                         Write-Host "JOB:   Triggering $($action.Name) (ID: $($action.ID)) via ccmexec.exe."
                         try {
                             # Start-Process requires elevation for this to succeed
                             ...
                             if ($process.ExitCode -ne 0) {
                                 Write-Warning "JOB:     $($action.Name) via ccmexec.exe finished with exit code $($process.ExitCode). (Requires elevation)"
                             } else {
                                 Write-Host "JOB:     $($action.Name) triggered via ccmexec.exe (Exit Code 0)."
                             }
                         } catch {
                             # Catch errors like Access Denied if not elevated
                             Write-Warning "JOB:     Failed to execute ccmexec.exe for $($action.Name): $($_.Exception.Message)"
                             $execMethodSuccess = $false # Mark exec method as failed
                         }
                     }
                     # If execution didn't throw errors, consider it attempted successfully
                     if ($execMethodSuccess) {
                         $overallSuccess = $true
                         Write-Host "JOB: Finished attempting actions via ccmexec.exe." -ForegroundColor Green
                     } else {
                         Write-Warning "JOB: One or more actions failed to execute via ccmexec.exe (likely requires elevation)."
                     }
                 } else {
                     Write-Warning "JOB: Fallback executable not found at $ccmExecPath."
                 }
             }

             # Final Status Logging
             if ($overallSuccess) {
                 Write-Host "JOB: CM actions attempt finished. At least one method appears to have triggered actions successfully." -ForegroundColor Green
             } else {
                 Write-Warning "JOB: CM actions attempt finished, but neither CIM nor ccmexec.exe methods could be confirmed as fully successful (check for elevation)."
             }
    ...
    try { # OUTER try for window loading
        ...
        $controls.lvwFiles.Add_PreviewMouseLeftButtonUp({
            ...
            if ($e.OriginalSource -is [System.Windows.Controls.CheckBox]) {
                ...
                # Use InvokeAsync for responsiveness
                $sender.Dispatcher.InvokeAsync([action]{
                    Write-Host "Executing scheduled UpdateRequiredSpaceLabel."
                    & $script:UpdateRequiredSpaceLabel -ControlsParam $controls
                }, [System.Windows.Threading.DispatcherPriority]::ContextIdle) | Out-Null
            }
        })
